...
const handleChangeVolume = (el) => {
  const { value } = el;
  document.querySelector("#volumeValue").innerText = value;
  window.electronAPI.changeVolume({ volume: value });
};
  ...
  try {
    ...
    PlaybackList.forEach((playback) => {
      ...
      document
        ...
        .setAttribute("value", defaultVolume);
      document.querySelector("#volumeValue").innerText = defaultVolume;
    });
  } catch (error) {
...
// 创建设备元素
const createDeviceElement = (id, name, isDisabled) => {
  const div = document.createElement("div");
  div.className = "bg-white p-4 rounded-lg shadow mb-4";
  div.innerHTML = `
    <div class="flex items-center justify-between audio-device">
    ${name}
  ...
  div.addEventListener("click", (event) => {
    const checkbox = event.target.querySelector(".device-toggle");
    if (checkbox && checkbox.checked) {
      const id = checkbox.getAttribute("data-id");
      //如果选中了
      window.electronAPI.togglePlaybackDevice(id);
    }
  });
  return div;
};
...
volumeControl.addEventListener("change", (event) => {
  handleChangeVolume({ value: event.target.value });
});
